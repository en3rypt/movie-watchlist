...
const Card = props => {
  return (
    <div className='flex rounded-xl  sm:rounded-none h-[60vh]' onClick={console.log('clicked')}>
      <div className='p-10  flex-col shrink-0 max-w-[400px] bg-black hidden sm:flex'>
        ...
        <p className='text-white  pt-5 font-knait text-sm font-extralight'>
          {props.overview.length > 34
            ? parse(`${props.overview.slice(0, 354)}...<Link to="/"><a >Read More</a><Link>`)
            : props.overview}
        </p>
      </div>
      <div className='relative grow'>
        <div className='absolute bottom-2 left-2 w-1/2 z-[2] sm:hidden'>
        ...
        <div className='absolute top-0 bottom-0 -left-1 bg-gradient-to-r from-black h-full z-[1]  w-1/2'></div>
